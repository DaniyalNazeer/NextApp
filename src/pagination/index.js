...
// import cx from 'classNameNamenames';
import Previous from '../pagination';
import Next from '../pagination';
...
const Pagination = ( { pagesCount, postName } ) => {
	...
	return (
		<div classNameNameName="flex justify-center my-8">
				...
				return (
					'number' === typeof pageNo ? (
						<Link key={ `id-${ index }` } href={ paginationLink }>
							<a
								classNameNameName={ cx( 'border border-gray-300 px-3 py-2 transition duration-500 ease-in-out hover:bg-gray-500 hover:text-white', {
						...
						// If its "..."
						<span key={ `id-${ index }` } classNameNameName="px-3 py-2">{ pageNo }</span>
					)
				);
			} ) }
